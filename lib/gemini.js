...
EXTRAITS PERTINENTS : 4 phrases EXACTES copiées du CV/profil qui sont les plus en rapport avec la mission.
   → Ce sont des citations directes qui prouvent la pertinence du candidat.
   → Choisis les bullet points les plus impressionnants en lien avec le poste.
...

{
  ...
  "analysis": {
    ...
    "relevant_excerpts": ["Citation exacte du CV 1", "Citation exacte du CV 2", "Citation exacte du CV 3", "Citation exacte du CV 4"]
  }
}`;
}
...
export const GeminiClient = {
    ...
    async analyzeCandidate(payload, job, tuningWeights, modelOverride) {
        ...
        console.log(`[Gemini] Analyse candidat - Modèle: ${modelId}`);
...
function validateAnalysisResult(res) {
    return {
        ...
        analysis: {
            ...
            relevant_excerpts: Array.isArray(res.analysis?.relevant_excerpts) ? res.analysis.relevant_excerpts : []
        }
    };
}
